Vue.component('cool-table', {
    ...
    <el-pagination ref="pagination" :class="paginationClass" :style="paginationStyle" :page-size="pageSize" :small="small" :total="total" \
    :page-count="pageCount" :current-page="currentPage"  :layout="layout" :page-sizes="pageSizes" \
    :popper-class="popperClass" :prev-text="prevText" :next-text="nextText" :background="background" ></el-pagination></div>',
    props: {
        //classes & styles
        ...
        layout: { default: 'sizes, prev, pager, next, jumper, ->, total' },
        pageSizes: { type: Array, default() { return [5,10, 20, 30, 40, 50, 100] } },
        popperClass: String,
        prevText: String,
        nextText: String,
        background: Boolean,
    ...
    methods: {
        ...
        registerEvents: function () {
            ...
            buttons.forEach(function (button) {
                redirectEvents(button, _this, buttonEvents, 'button-')
            })
        }
            ...
            var tableEvents = ['select',
                ...
                'sort-change',
                'current-change']
            redirectEvents(table, this, tableEvents,'table-')
        ...
        autoHeight: function () {
            ...
            //important: 不要重复实例化同一段html，否则$refs的元素会被替换掉，导致以下高度计算失败
            ...
            //console.debug(parentHeight + ' - ' + buttonGroupHeight + ' - ' + customHeight + ' - ' + parentHeight + ' - 1' )
            //渲染计算的时候可能会四舍五入导致边框重叠，-1好看点。。。
        },
